...
- 升级 tracex v1.2.0（错误码大写）。

## [v1.1.2] - 2026-08-11

### 修复与质量

- lib/app、lib/service、lib/win32 覆盖率补至 100%：
  默认信号等待、事件日志写入、服务管理适配器与停止轮询分支补齐白盒测试；
- lib/service 移除真实 SCM 适配器层，改为接口适配（mgrOps/serviceOps），
  测试不再依赖服务管理器权限；
- lib/config 增加编译期类型断言；
...
- README / docs 与当前代码同步：版本状态、API 与架构文档更新；
- 清理过期规划文档（design/roadmap 等）。
...
- 示例服务入口（main）从根目录迁移至 `examples/service`，根目录仅保留 `lib/*` 公开子包；
- 新增 `examples` 独立模块并接入 CI/Release 测试与构建；
- 补齐 `lib/config` 测试。
...
- 依赖升级：tracex v1.0.4、errx v1.5.7、logx v1.3.4、validx v1.2.5、testx v1.4.5。
